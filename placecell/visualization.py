...
def plot_rate_map_1d(
    ...
    ax.fill_between(centers, rm, alpha=0.3, color="steelblue")
    ax.plot(centers, rm, color="steelblue", linewidth=1.5)
...
def plot_occupancy_preview_1d(
    ...
    if time_col in trajectory_1d_filtered.columns:
        t = trajectory_1d_filtered[time_col] - trajectory_1d_filtered[time_col].iloc[0]
        ax_traj.scatter(t, trajectory_1d_filtered[pos_col], s=0.5, alpha=0.3, color="steelblue")
    ...
    ax_traj.set_title(f"Tube trajectory ({len(trajectory_1d_filtered)} frames)")
