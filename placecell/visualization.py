...
def plot_place_cell_summary(
    ...
    -------
    pd.DataFrame
        Results with unit_id, SI, and p_value columns.
    """
    if plt is None:
        raise ImportError(
            "matplotlib is required for plotting. Install it with: pip install matplotlib"
    ...
    # Analyze each unit
    unique_units = df_filtered["unit_id"].unique()
    traj_frames = trajectory_df["beh_frame_index"].values
    results = []
    ...
    for unit_id in unique_units:
        ...
        results.append({"unit_id": unit_id, "SI": actual_si, "p_value": p_val})
        ...
        # Top row: 4 subplots (each 1/4 width)
        ax1 = fig.add_axes([0.02, 0.45, 0.22, 0.50])  # Max projection
        ax2 = fig.add_axes([0.27, 0.45, 0.22, 0.50])  # Trajectory
        ax3 = fig.add_axes([0.52, 0.45, 0.22, 0.50])  # Rate map
        ax4 = fig.add_axes([0.77, 0.45, 0.21, 0.50])  # SI histogram

        # Bottom row: Trace (full width)
        ax5 = fig.add_axes([0.05, 0.08, 0.90, 0.30])  # Trace
        ...
        if max_proj is not None:
            ...
            if footprints is not None:
                try:
                    # Get centroid of this unit's footprint
                    unit_fp = footprints.sel(unit_id=int(unit_id)).values
                    y_coords, x_coords = np.where(unit_fp > unit_fp.max() * 0.3)
                    if len(x_coords) > 0:
                        cx, cy = np.mean(x_coords), np.mean(y_coords)
                        ax1.scatter(
                            [cx], [cy], c="red", s=100, marker="o", edgecolors="white", linewidths=2
                        )
                        # Draw contour
                        ax1.contour(unit_fp, levels=[unit_fp.max() * 0.3], colors="red", linewidths=1)
                except Exception:
                    pass
            ax1.set_title(f"Unit {unit_id} position")
        else:
            ax1.text(
        ...
        # 4. SI histogram
        ax4.hist(shuffled_sis, bins=15, color="gray", alpha=0.7, edgecolor="black")
        ax4.axvline(actual_si, color="red", linestyle="--", linewidth=2)
        ax4.set_title(f"SI: {actual_si:.2f}, p={p_val:.3f}")
        ax4.set_xlabel("SI (bits/s)")
        ax4.set_ylabel("Count")
        ...
        if traces is not None:
            try:
                ...
                ax5.set_title(f"Unit {unit_id} trace")
                ax5.set_xlabel("Time (s)")
                ax5.set_ylabel("Fluorescence")
            except Exception as e:
                ax5.text(
                    0.5, 0.5, f"Could not plot trace: {e}", ha="center", va="center", transform=ax5.transAxes
                )
        else:
            ax5.text(0.5, 0.5, "No trace data", ha="center", va="center", transform=ax5.transAxes)
            ax5.set_xlabel("Time (s)")
        ...
        # Save figure
        out_path = output_dir / f"unit_{unit_id}_summary.png"
